...
def search_venues():
  ...
  for venue in venues:
    data.append({
      "id": venue.id,
      "name": venue.name,
      "num_upcoming_shows": len(db.session.query(Show).filter(Show.venue_id==venue.id).all())
    })
  response = {
    "count": len(venues),
    "data": data
...
def show_venue(venue_id):
  ...
  return render_template('pages/show_venue.html', venue=showVenue)
...
def search_artists():
  ...
  for artist in artists:
    data.append({
      "id": artist.id,
      "name": artist.name,
      "num_upcoming_shows": len(db.session.query(Show).filter(Show.artist_id==artist.id).all())
    })
  response = {
    "count": len(artists),
    "data": data
...
def show_artist(artist_id):
  ...
  print(showArtist)
  return render_template('pages/show_artist.html', artist=showArtist)
